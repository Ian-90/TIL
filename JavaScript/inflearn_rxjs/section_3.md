...
* merge - 두 스트림을 순서관계없이 병합
  ...
  const { merge, interval, fromEvent } = rxjs
	...
	merge(interval$, click$).subscribe(console.log) // interval + click시 click
	...
	merge(intv1$, intv2$, intv3$, intv4$, intv5$, 3) // 3개씩 머지(마지막 인자 개수만큼 머지)
	.subscribe(console.log)
  ```
...
* concat - 스트림을 순서대로 이어붙임
	...
	concat(intv1$, intv2$, intv3$) // 'INTERVAL 1 'INTERVAL 1 'INTERVAL 1 'INTERVAL 2 'INTERVAL 2 'INTERVAL 2 'INTERVAL 3 'INTERVAL 3 'INTERVAL 3
	.subscribe(console.log)
	...
	concat(interval$, click$).subscribe(console.log) // 첫번째 인자 값을 구독 후, 다 끝나면 두번째 인자 값을 구독
	```

* mergeMap - 두 번쨰 인자로 몇 개의 스트림을 동시 진행할 것인지 설정 가능
	```js
	const { interval, fromEvent } = rxjs
...
* concatMap
	...
	// mergeMap과 다르게 늘 동일한 순서로 이름들이 반환
	of(3, 15, 4, 9, 1, 7).pipe(
...
### 6.2 ~MapTo 연산자들 : 값은 두번쨰 스트림에서만 발행
